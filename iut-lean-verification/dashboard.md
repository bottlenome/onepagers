...
| M72 2 変数法則への 1 変数代入（逆元キャンペーン第一層） | **形式群の逆元 [-1] 級数キャンペーンの基盤**: 2 変数級数 F(X,Y) に 1 変数級数 P(T)・Q(T) を代入する **ps21Comp**（F(P,Q)_n = Σ_{b,a≤n} F_{b,a}·(P^a Q^b)_n、P(0) = Q(0) = 0 で真の代入と一致）を構成——**1 変数冪の対角下消滅 psPow_low**（(Q^k)_n = 0、n < k。M59 の f 特化版の一般化）、冪積の低次消滅 psPowPow_low、**打ち切り境界の付け替え ps21Comp_pad**（M70e の pad3 の 1 変数ミラー）、**定数項 = F₀₀（無条件）と一次係数 master 補題 ps21Comp_lin**（F(P,Q)₁ = F₀₁·P₁ + F₁₀·Q₁、4 項展開で (0,0)・(1,1) 項は消滅）。逆元の本体（ι = ltSol(−1)・F(X, ι(X)) = 0）は次層以降。選択公理不使用 | 実体構成 |
...
| M72: 2 変数法則への 1 変数代入 ps21Comp（1 変数冪の対角下消滅・冪積の低次消滅・pad・定数項/一次係数 master 補題。**逆元 [-1] 級数キャンペーン第一層**） — 第57弾 | done（choice なし） |
