/-
  ...
  ★ 追加の正直な限定（独立敵対監査 2026-07-21・§4 に従い削除しない）:
    本モジュールは ℚ(√d) の積公式を **M351F `pf_product_formula` へ還元**して証明するが、
    その M351F が **循環的**である（「log|x|_∞」が有限部と同じ付値ベクトルから定義され、
    `logp` は仮説ゼロの自由パラメータ。詳細は `IUT/ProductFormula.lean` の訂正ブロック）。
    したがって本モジュールの「一般数体の忠実な部分ケース」という主張は
    **循環を継承しており**、実の二次体積公式を確立してはいない。冒頭の [実／本物] 分類は
    この点で過大であり、本注記をもって訂正する。忠実な還元先を実の積公式
    `IUT/B5ProductFormulaQ.lean` へ張り替えるのが本来の修復であり、別ラウンドの課題。
-/
