...
/-- **定理 (tfd-9a・本丸): Fermat 3 次曲線環 Q = K[u,v]/(u³+v³−1) は零因子を持たない**
    （＝ u³+v³−1 は K[u,v] の素元）。★**正直な訂正（独立敵対監査 2026-07-21）**: 当初ここに
    「被覆は既約＝**連結**」と書いたのは限定 2(ii) より**強く過大**であった。本モジュールが
    確立したのは **環論的な整域性（＝素イデアル）のみ**であり、Spec/位相/局所環/付値は
    リポジトリ全体に存在しないため、「Spec Q が既約＝連結」という**幾何的主張は未達**である
    （監査の位置づけ: 実質的な半分は達成、幾何的言明は名目上未達で浅い）。
