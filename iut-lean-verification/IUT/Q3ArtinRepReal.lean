/-
  ...
  **本モジュールの真水（NEW・監査対象の新規実定理）**:
   ...
   (R8) 正則表現の **直和分解 V_reg = V₀⊕V_χ⊕V_{χ²}**（存在＋一意性）——Σ_ρ が
        3 本の階数1自由部分加群への直和分解（**訂正: 既約分解ではない・下記参照**）。
  ══════════════════════════════════════════════════════════════════
  ★ 正直な訂正（独立敵対監査 2026-07-21・§4 に従い削除も弱化もしない）★
   1. **「C₃ の既約全体をちょうど 1 回ずつ」は過大**。本モジュールは **既約性を定義も
      証明もしていない**（`q9ar_regular_decomp` は 3 本の階数1自由部分加群への直和分解
      であって既約分解ではない。O_{L₂}-加群としての既約性は実際には偽）。定理の
      statement 自体は「既約」の語を使っておらず、過大なのはヘッダ散文のみ。
   2. **`q9ar_G_eq_G0` は content-free**。実体は `⟨fun _ => q9ar_G_all …, fun _ => q9ar_G_all …⟩`
      ＝両辺が独立に真の iff であり **何も拘束しない**（R8 で B4 が咎められたのと同種）。
      しかも grep で **下流未使用**と確認された。したがって「重み |G_i|/|G₀| を実事実で
      置き換えた」という主張は**支持されない**——和 c0+c1+c2 は単に重み無しで、
      この 1 例でたまたま正しい値になっているだけ。同様に `q9ar_triv_basis`・
      `q9ar_chi2_basis`・`q9ar_regular_decomp_unique`・`q9ar_conductor_tail` も未使用。
   3. **Σ_ρ a(ρ)·dim ρ の「·dim ρ」は形式化されていない**（算術に一度も現れない。
      dim=1 は上記 basis 補題に支えられた散文）。また **正則表現自身の conductor は
      未計算**（0/1 の codim spec は codim 2 を表現できない）ゆえ、和は V_reg から
      読み出したのではなく **3 本の線から手で組み立てた**ものである。
   4. codim の**数値**は stipulative: 実計算されるのは V^{G_i} ∈ {0, V} であり、
      そこから 0/1 への読み替えは `q9arIsCodim` の定義による（length/rank 理論なし）。
   5. 監査の vacuity floor: `Unit` 台の `q9arRep` は全公理を満たし codim 0 と 1 の
      **両方**を満たす。判別性は record の外側の `q9ar_*_nonzero` 定理が担保している
      （欠陥ではないが、guard が構造体の外にあることは明記する）。
   6. 限定 1 の「v_M 無し」は**やや陳腐化**: B1 に実 v_M（`q9v`・i_G(σ)=3）が既に在り、
      本モジュールが使っていないだけである。
   監査結果 **B3 0.34 → 0.42（+0.08）**。make-or-break の codim spec 判別性は
   監査自作の falsifier 7 本すべてを通過（PASS）。
  ══════════════════════════════════════════════════════════════════
  ...
  **消費（再主張しない・二重計上回避）**:
  ...
  3. **Artin 導手の重み |G_i|/|G₀| は有理数として形式化していない**。i≤2 では
     G_i = G₀（集合として一致・`q9ar_G_eq_G0`——**訂正: この iff は content-free かつ下流未使用**）ゆえ重み 1、i≥3 では codim=0 ゆえ項が
