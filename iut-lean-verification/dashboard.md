...
| M4 テータ値評価 | 値・次数簿記層を形式化: Gaussian 総次数の閉形式、素朴評価⟹RC評価/多輻的表現と非両立、膨張込み評価⟹M7計算 | 算術核 100% / 理論 ~25% |
| M5 定理3.11 多輻性 | **statement を原文から読み取り出力仕様を形式化**: (i)(Ind1)(Ind2)・(ii)(Ind3)・(iii) をインターフェース化、系3.12 の証明本体を機械化（公理ゼロ）、厳密評価の障害・膨張の必然性・仕様の充足可能性を証明。**構成**（遠アーベル復元等）は未形式化 | ~20%（構成 0%） |
...
**総括**: 「IUT は正しい」**~19%**（ページ規模重み付け: 233/1200 ≈ 19%）、
