...
| [IUT 系3.12 の Lean 形式検証](iut-lean-verification/index.html) | doc | done | 原論文PDFを一次資料に論争の形式骨格を Lean 4 で機械検証（総合証明率 ~12%）。Scholze-Stix読みでは矛盾、望月読みでは系3.12の独立性を証明。M1 公理非対称性・M2 ラベル対称性・M3 log-theta格子・M7 Szpiro条件付き導出・M8 ABC⟹漸近フェルマーも形式化済み（sorry なし） |
