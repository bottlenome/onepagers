/-
  ...
  **有理数になる類は D1 で検出できない**。したがって rank≥3 には ℤ₃ 側の立方障害
  （mod 9・D2）が必要である（設計書 K2a の「gr 簿記のみ」という見積りは
  この点で楽観的だった）。本モジュールは D1・D2 の両方を実装する。

  ★**正直な訂正（独立敵対監査 2026-07-21）**: 上の「**原理的に必要**」は**過大**であり
  訂正する。正しくは「**D1 型の判別器族の内部では**必要」であって、**絶対的に必要では
  ない**——監査の指摘どおり、B2 で既に計上済の crux `q9lr_four_not_norm`（4∉N_{M/L₂}(U_M)）
  に既存の `q9ps_normBase_embed`（N(embed n)=n³）・`q9wr_embed_unit` を合わせれば
  `q9c3_four_noncube` は mod-9 機構なしに数行で従い（u³=4 ⟹ N(embed u)=4 が矛盾）、
  同様に `q9rc_foursq_not_norm` から [16] が、ζ₃=N(ζ₉)（`q9tl_normBase_zeta9`）から
  混合 4ζ₃^j・16ζ₃^j が従う。すなわち **本モジュールの単数側 8 標的のうち 6 は
  B2 の既計上結果から既に含意されており**、新規なのは D1/D2 という Lean 上の経路のみ。
  監査はこれを理由に K2a の寄与を小さく見積もった（B6 全体で +0.06→0.24）。
