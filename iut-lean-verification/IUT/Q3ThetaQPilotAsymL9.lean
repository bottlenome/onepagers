/-
  ...
  ★ 正直な訂正（独立敵対監査 2026-07-21・§4 に従い削除も弱化もしない）★
   1. **見出し「Θ 冪＝真正 q 冪」は定義的であり、監査に機械的に反証された。**
      監査は本ファイル自身の 3 行証明スクリプトで
        `probe_any_k (k : Nat) : (q9tl3^k)^9 = q9tlQ^k`
      をコンパイルし、`q9tq_thetaVal_pow9 j := probe_any_k (j*j)` が型検査を通ることを
      示した。すなわち **j² は装飾**であり、見出しは**任意の指数**で成立する。
      `q9tlQ := (54, u₆⁹)` が `q9tl3 := (6, u₆)` の成分毎 9 乗として**定義**されている以上、
      「Θ が真正 q 冪に着地する」は可換群の `(x^a)^9 = (x^9)^a` に還元する。
      したがってヘッダ冒頭の「監査要求への直答 (d-1) …Yes」は**過大**であり撤回する。
      さらに `ThetaValueLtor.thLtorExp_sq`・`GaloisTheta.galTh_norm_qpower` に
      同じ定義的所作が **既に E2(0.50) の中にある**。`q9tq_thetaVal_npow_ne_one` も
      「付値 6j²k ≠ 0」であって 3 の冪なら何でも真——theta 固有の事実ではない。
      Θ には **theta 関数由来が無い**（`q9tq_theta_from_cocycle` は目標に当たるよう
      逆算されたレシピである）。
   2. `q9tq_g3_pow` の二次指数 Tri(n) が cocycle からの**真の帰納導出**である点は
      監査も確認した。ただし監査は byte-identical な証明で
        `probe_generic_quad (w) : ((1,−1,w))ⁿ = (w^{−Tri n}, −n, wⁿ)`
      を示し、本補題がその **w := 3 インスタンス**＝**theta 群入力ゼロの generic
      Heisenberg/単項群補題**であることを立証した（membership はどこでも仮説になっておらず、
      すべて出力側である）。
   3. **未申告の重複**: `q9tq_tri_sq` は `GaussianVolume.sq_exponent` の再証明である。
   4. E4 の真の標的「実 theta の **Kummer 理論**」に対して本モジュールは
      **Kummer 類ゼロ・Galois 作用ゼロ・コホモロジーゼロ**である。
   **真に獲得したもの**（監査認定）: パイロット比較 δ = q⁻³ ≠ 1（中心・theta 群内・
   valuation −162）は、q9tk の退化（ζ₉⁹=1）と違い a = −1 枝上の実関係に基づく
   **リポ初の非零 theta 側 vs q 側 witness**。監査結果 **E4 0.03 → 0.08（+0.05）**。
  ══════════════════════════════════════════════════════════════════
