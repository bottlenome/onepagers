...
## 状態: done (index.html 完成)

## 構成

```
teichmuller/
├── index.html          # 解説ワンページャー (10タブ構成)
├── dashboard.md        # このファイル
├── pdf/                # 原論文PDF (6本)
└── notes/
...
### index.html のタブ構成 (done)

| タブ | 内容 |
|------|------|
| 概要 | 30秒/3分/全体像の図/主要構成物 |
| なぜ？ | 10問のQ&A（折りたたみ）+ 「2つの次元」の正体 |
| 構造 (4巻) | IUT I–IV の各巻の詳細 + 全体フロー図 |
| 例え | 翻訳/地図のアナロジー + 個別概念の例え + ガウス積分 |
| 数値例 | 5進数での足し算と掛け算の絡み合い |
| 応用 | Vojta/ABC/Szpiro/間接帰結 + 影響範囲図 |
| 歴史 | 前史/望月の研究歴/IUT公開後 のタイムライン + 知的系譜 |
| 論争 | 三者の立場/Scholze-Stix/Joshi/現状 |
| 用語集 | 基礎/数論幾何/IUT固有/古典との対応 |
| 資料 | PDF6本/縦読みノート10本/横読みノート8本/一般向け資料 |
...
- index.html はタブ式SPA。ノート群へのリンクと併用する読み口
