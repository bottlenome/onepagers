...
| M5: 定理3.11 の**構成**の完全形式化 | 4基盤の骨格（M9–M12）は done。**Frobenioid の圏論的実体と数体型データでの充足は M51F で base 圏一点の範囲を形式化済み**（因子圏・次数関手・実データによる M12 充足。poly-isomorphism は M53F、分裂（単数）は M55F、base 圏上のファイバー構造は M57F が離散 base、M61F が分岐指数つき塔 base（単一素点）の範囲で形式化済み。複数素点への分解・惰性・realification は未達）。残り（環構造復元の本体・p進テータ関数の関数等式）は mathlib 規模の数論幾何ライブラリが必要。世界の誰も達成していない |
