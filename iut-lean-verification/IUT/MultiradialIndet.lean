/-
  ...
  この 3 不定性を **RReal 上の本物の群作用**として組織化する:
    ...
    * (Ind1) = ±1/単数自己同型群 Z/2（`Bool`, `Bool.xor`）が付値ゼロを経て作用
      （単数は v=0、`mindSignVal ≡ 0`）。
    * (Ind2) = 水平⊗ 単数トーソル Z/2 が同様に付値ゼロを経て作用（`mindUnitVal ≡ 0`）。

  ══════════════════════════════════════════════════════════════════
  ★ 正直な訂正（D 柱 詳細化ラウンド 2026-07-21・親が独立に code で確認・§4 に従い削除しない）★
    上の (Ind1)/(Ind2) を「**値を保存する本物の有限群作用**」と書いていたのは **過大**であり
    撤回する。実際には `mindSignVal (_ : Bool) : Int := 0`（147 行）・`mindUnitVal ≡ 0` は
    **恒等的にゼロ**であり、`mindValAct logq v 0 V = realAdd V (logVolLocal logq v 0) ≈ V`
    すなわち **作用そのものが恒等写像**である。したがって「値を保存する」は作用が非自明
    だから成り立つのではなく、**作用が何もしないから自明に成り立つ**。不変性定理
    （`mind_comp_invariant` 等）および群作用律も、この自明性から従っている。
    Ind1/Ind2 の**本物の**不定性（実 ℤ₃^× トーソル・実単数の作用）は**未実装**であり、
    これが D6 が台帳で 0.00 のままである理由の一つである（過大主張を台帳側では
    していない点は保たれている）。
    実対象で Ind を建てる計画は `audit/pillar-D-next-step-detail-2026-07-21.md` の D-6-1/2 参照。
  ══════════════════════════════════════════════════════════════════
